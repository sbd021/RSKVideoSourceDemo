...
/**
 * Remote video stream downscale level.
 */
enum REMOTE_VIDEO_DOWNSCALE_LEVEL {
  ...
  /**
   * Note that the higher the level is, the more serious the downscale is.
   */
...
struct VideoCanvas {
  ...
  VideoCanvas() : view(NULL), renderMode(media::base::RENDER_MODE_HIDDEN),
  ...
  VideoCanvas(view_t v, media::base::RENDER_MODE_TYPE m, VIDEO_MIRROR_MODE_TYPE mt, uid_t u)
      : view(v), renderMode(m), uid(u), isScreenView(false), priv(NULL), priv_size(0),
        sourceType(VIDEO_SOURCE_CAMERA) {}
  VideoCanvas(view_t v, media::base::RENDER_MODE_TYPE m, VIDEO_MIRROR_MODE_TYPE mt, user_id_t u)
      : view(v), renderMode(m), uid(0), isScreenView(false), priv(NULL), priv_size(0),
        sourceType(VIDEO_SOURCE_CAMERA) {}
};
...
/**
 * Generate credential
 * @param [out] credential The content of the credential.
 * @param [in] path The path of saving the credential file.
 * @return The description of the error code.
 * @note For license only.
 */
AGORA_API int AGORA_CALL agora_genCredential(agora::util::AString &credential, const char *path);
 ...
 * returned.
 * @param [in] cert The content of the certificate.
 * @param [in] path The path of saving the credential file.
 * @return The description of the error code.
 * @note For license only.
 */
AGORA_API int AGORA_CALL agora_verifyCertificate(const char *cert, const char *path);

/**
 * @brief Implement the agora::LicenseCallback and create a object to receive callbacks of license
 *
 * @param [in] callback The object of agora::LiceseCallback
 * @return none
 */
AGORA_API void agora_registerLicenseCallback(agora::base::LicenseCallback* callback);

/**
 * @brief unregister the object of LicenseCallback, and then won't receive callbacks
 *
 * @return none
 */
AGORA_API void agora_unregisterLicenseCallback();
